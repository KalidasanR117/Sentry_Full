    ...
    for det in violence_results:
        if det.get("class") == "violence":
            try:
                ...
                color = (0, 0, 255)
                cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), color, 2)
                cv2.putText(annotated_frame, label, (x1, y2 + 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
            except Exception as e:
...
def process_video():
    ...
    try:
        for frame_count, (frame, _) in enumerate(capture_frames(save_path)):
            ...
            violence_prediction = (
                violence_results[0]["class"] if violence_results else "unknown"
            )
            ...
            if severity in ["danger", "suspicious"]:
                ...
                screenshot_path = os.path.abspath(
                    os.path.join(SCREENSHOT_DIR, f"upload_{time.time()}.jpg")
                )
                ...
                yolo_strings = [
                    f"{det['class']} ({det.get('confidence', 0.0):.2f})"
                    for det in yolo_results
                ]
                event_data = {
                    "frame": frame_count,
                ...
                if severity == "danger" and (
                    time.time() - local_last_alert_time
                ) > ALERT_COOLDOWN:
        ...
        if video_specific_events:
            ...
            print(f"[DEBUG] Saving upload report to: {pdf_path}")
            ...
            if os.path.exists(pdf_path):
                ...
                print(f"[ERROR] Upload report not created at {pdf_path}")
        return jsonify(
            {
                "status": "Video processed and report generated.",
                "events_found": len(video_specific_events),
            }
        )
    except Exception as e:
...
def camera_analysis_loop_local(source=0):
    ...
    try:
        for frame_count, (frame, _) in enumerate(capture_frames(source)):
            if not is_camera_processing.is_set():
                break
            yolo_results = detect_from_frame(frame, threshold=YOLO_CONF_THRESHOLD)
            violence_results = run_violence_detection(frame)
            violence_prediction = (
            ...
            if severity in ["danger", "suspicious"]:
                screenshot_path = os.path.abspath(
                    os.path.join(SCREENSHOT_DIR, f"live_local_{time.time()}.jpg")
                )
                try:
                ...
                if screenshot_path:
                    ...
                    alert_message = {
                        "message": f"{severity.capitalize()} Detected: {', '.join(yolo_classes)} & {violence_prediction}",
                        "type": "error" if severity == "danger" else "warning",
                        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    }
                    yolo_strings = [
...
def camera_analysis_loop_network(source=0):
    ...
    try:
        ...
        while is_camera_processing.is_set():
            ...
            if severity in ["danger", "suspicious"]:
                screenshot_path = os.path.abspath(
                    os.path.join(SCREENSHOT_DIR, f"live_network_{time.time()}.jpg")
                )
                try:
                    save_success = cv2.imwrite(screenshot_path, annotated_frame)
                    if not save_success:
                        print(
                            f"⚠️ WARNING: Failed to save NETWORK screenshot to {screenshot_path}"
                        )
                        screenshot_path = None
                except Exception as e:
...
def start_camera_analysis():
    ...
    if camera_thread is None or not camera_thread.is_alive():
        ...
        target_loop = (
            camera_analysis_loop_network if is_network_stream else camera_analysis_loop_local
        )
        ...
        camera_thread = threading.Thread(
            target=target_loop, args=(video_source,), daemon=True
        )
        camera_thread.start()
        print(
            f"Starting analysis with {'NETWORK' if is_network_stream else 'LOCAL'} loop for source: {video_source}"
        )
...
def stop_camera_analysis():
    ...
    if camera_thread and camera_thread.is_alive():
        ...
        if camera_thread.is_alive():
            print("⚠️ WARNING: Camera thread did not stop cleanly.")
        camera_thread = None
...
def generate_camera_report():
    with lock:
        if not camera_report_buffer:
            return jsonify({"status": "No events to report."}), 404
    ...
    try:
        ...
        print(f"[DEBUG] Saving live report to: {pdf_path}")
        ...
        if os.path.exists(pdf_path):
            ...
            return jsonify(
                {
                    "status": f"Report generated and sent with {len(events_to_report)} events.",
                    "filename": os.path.basename(pdf_path),
                }
            )
        else:
            print(f"[ERROR] Live report not created at {pdf_path}")
            return jsonify({"status": "Report generation failed."}), 500
    except Exception as e:
        print(f"!!! [Report Generation] AN ERROR OCCURRED: {e} !!!")
        return jsonify(
            {"status": f"An error occurred during report generation: {e}"}
        ), 500
...
def download_report(filename):
    ...
    if os.path.exists(file_path):
        ...
        print(f"[ERROR] Requested missing file: {file_path}")
        return (
            jsonify({"error": "File not found.", "expected_path": file_path}),
            404,
        )
...
def get_alerts():
    with lock:
        return jsonify(camera_alert_buffer[-5:][::-1])
