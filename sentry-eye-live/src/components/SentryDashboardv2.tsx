...
// ===================================================================
// Sentry AI: Modern Dashboard UI
//
// A complete redesign of the frontend, focusing on a professional,
// high-tech "Security Operations Center" aesthetic.
//
// To use:
// 1. Place this file in your `sentry-eye-live/src/components/` folder.
// 2. Update your main page (e.g., `App.tsx` or `page.tsx`) to import
//    and render `<SentryDashboardModern />` instead of the old one.
// ===================================================================

const API_URL = import.meta.env.VITE_REACT_APP_API_URL || "http://127.0.0.1:5000";
type VideoSource = "none" | "camera" | "upload";

// --- Mock Data for the decorative analytics chart ---
...
export const SentryDashboardModern = () => {
    // --- State Management (Identical to your original dashboard) ---
    ...
    // --- API Functions (Identical to your original dashboard) ---
    useEffect(() => {
    ...
    const startNetworkStream = async () => {
        if (!networkUrl.trim()) return alert("Please enter a network stream URL.");
        setIsConnecting(true);
        try {
    ...
    // --- JSX Structure for the New Design ---
    return (
        <div className="bg-gray-900 text-gray-200 min-h-screen p-4 flex flex-col font-sans">
            ...
            <header className="flex items-center justify-between mb-4 px-2">
                ...
                <div className="flex items-center gap-4">
                    <Badge variant="outline" className="border-green-400/50 text-green-300 bg-green-900/30">
                        <div className="w-2 h-2 bg-green-400 rounded-full mr-2 animate-pulse" />
                        System Active
                    </Badge>
                    <Button variant="outline" className="text-gray-300 border-gray-700 hover:bg-gray-800 hover:text-white" onClick={() => (window.location.href = "/reports")}>
            ...
            <main className="flex-grow grid grid-cols-12 gap-4">
                ...
                <section className="col-span-6 flex flex-col">
                    ...
                    {source === 'upload' && (
                         <div className="flex items-center justify-end gap-2 p-4 bg-gray-800/50 border-t-0 border-x border-b border-gray-700 rounded-b-lg">
                ...
                <aside className="col-span-3 flex flex-col gap-4">
                    <Card className="bg-gray-800/50 border-gray-700 flex-grow flex flex-col">
                        ...
                        <CardContent className="flex-grow space-y-2 overflow-y-auto">
                            {alerts.length === 0 ? (
                                <p className="text-gray-500 text-sm text-center pt-8">No alerts yet.</p>
                            ) : (
                                alerts.map((alert, idx) => (
                                    <div key={idx} className={`p-2 rounded-md text-sm ${alert.type === "warning" ? "bg-amber-900/50 text-amber-300" : "bg-red-900/50 text-red-300"}`}>
                    ...
                    <Card className="bg-gray-800/50 border-gray-700">
                        ...
                        <CardContent className="h-40">
                             <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={mockAnalyticsData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" stroke="#4A5568" />
                                    <XAxis dataKey="name" stroke="#A0AEC0" fontSize={12} />
                                    <YAxis stroke="#A0AEC0" fontSize={12}/>
                                    <Tooltip contentStyle={{ backgroundColor: '#1A202C', border: '1px solid #4A5568' }} />
                                    <Line type="monotone" dataKey="events" stroke="#FBBF24" strokeWidth={2} />
                                </LineChart>
                            </ResponsiveContainer>
                        </CardContent>
                    </Card>
